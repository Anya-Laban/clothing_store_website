...
import {ShowFilters, ApplyingFilters, FiltredGoogs,HeaderAplingFilters,SearchGoods,ShowPagination,ShowGoodsWithPagination,GoodsWithPagination, AuditCardClick} from "./functions.js"
...
document.querySelector("#currentPage").insertAdjacentText("afterbegin",currentPage)
...
const [...filterContentBoxes] = document.querySelectorAll(".filter-content__box")
...
filterContentBoxes.forEach((el)=>{
    el.addEventListener("click", ()=>{
       ...
       ShowGoodsWithPagination(FiltredGoogs(allCards, clientFilter), numRows, currentPagePagination, document.querySelector(".main-section__goods"));
       ShowPagination(paginationEl,FiltredGoogs(allCards, clientFilter), numRows, currentPagePagination, lastPagePagination)
       AuditCardClick(document.querySelectorAll(".card"),GoodsWithPagination(FiltredGoogs(allCards, clientFilter), numRows, currentPagePagination));
    })
})
...
searchEl.addEventListener("change",(e)=>{
    ...
    SearchGoods(allCards,e.target.value,document.querySelector(".main-section__goods"),searchEl, numRows, currentPagePagination,lastPagePagination, paginationEl)
    AuditCardClick(document.querySelectorAll(".card"),GoodsWithPagination(FiltredGoogs(allCards, clientFilter), numRows, currentPagePagination));
})
...
paginationEl.addEventListener("click", (e) => {
    const countPages = Math.ceil(FiltredGoogs(allCards, clientFilter).length/numRows);
    if(e.target.dataset.pag !== undefined && e.target.dataset.pag !== NaN){
        ...
        if(e.target.dataset.pag ==="start"){
            ...
            localStorage.setItem('currentPagePagination',Number(e.target.dataset.pag));
        }
    }
    currentPagePagination = localStorage.getItem('currentPagePagination');
